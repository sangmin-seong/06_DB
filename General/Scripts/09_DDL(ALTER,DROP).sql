...
-- EX) CHAR(3) 컬럼에 'ABC' 저장된 상태,
--     -> CHAR(1)로 수정하려고 하면 오류 발생 ==> JAVA 강제 형변환 불가!!!  
...
SELECT 
	...
	LENGTHB(DEPT_TITLE) "문자열 BYTE 크기"
FROM DEPT_COPY;
...
SELECT * FROM DEPT_COPY; -- 'D0' 추가

-- DEPT_COPY 테이블에 컬럼 추가하는 DDL 구문
ALTER TABLE DEPT_COPY
ADD (DEPT_TITLE VARCHAR2(30));

SELECT * FROM DEPT_COPY; -- DEPT_TITLE 추가
...
-- 해결방법 1) 참조하는 자식 테이블(TB2)을 먼저 삭제하거나 FK 제약조건을 삭제


-- 해결방법 2) CASCADE CONSTRAINTS 옵션 사용하기
