...
AS 
	...
	ORDER BY 사번 ASC;
-- FK 제약조건을 가진 컬럼을 이용하여 JOIN을 진행한다.

-- VIEW를 이용해서 조회하기
SELECT * FROM V_EMP;

-- V_EMP에서 대리 직원들을 이름 오름차순
...
-- V_DCOPY2 VIEW 이용, INSERT
INSERT INTO V_DCOPY2
VALUES('D0', 'L2');
--> 가상의 테이블 VIEW인데 INSERT 성공!

-- INSERT 결과 확인 - V_DCOPY2
SELECT * FROM V_DCOPY2;
-- D0	L2

-- INSERT 결과 확인 - DEPT_COPY2
SELECT * FROM DEPT_COPY2;
-- D0	NULL	L2

-- VIEW에 INSERT를 수행했지만 VIEW를 만들때
-- 사용한 원본 테이블에 값이 INSERT 됨을 확인!!!!
...
INCREMENT BY 5 -- NEXTVAR 호출 시, 5씩 증가
MAXVALUE 150   -- 증가 가능한 최대값 150
NOMINVALUE 		 -- 최소값 없음
NOCYCLE 			 -- 반복 안함
NOCACHE; 			 -- 미리 만들어 둔 시퀀스 번호 없음 

-- 시퀀스 테스트할 테이블 생성
CREATE TABLE TB_TEST(
	TEST_NO NUMBER PRIMARY KEY,     -- TEST_NO 라는 컬럼에 숫자만 저장하는 
	TEST_NAME VARCHAR2(30) NOT NULL -- 30BYTE 짜리 공간 만듦, NULL 허용하지 않음 
);
...
WHERE TEST_NAME = '짱구';
--> '짱구'의 TEST_NO 값을 150까지 증가시키고 나서
--> 다시 수행해보면
-- ORA-08004: 시퀀스 SEQ_TEST_NO.NEXTVAL
--						exceeds MAXVALUE은 사례로 될 수 없습니다
