...
/****************************************************************/
-- 기본적으로 "서브쿼리"를 먼저 해석!!!
----> 서브쿼리 결롸글 이요해서 "메인쿼리" 해석			   

-- 단, 상화 연관 서브쿼리는 순서가 반대!!!(메인 -> 서브)
/*****************************************************************/
-------------------------------------------------------------------
...
-- 1) 사수에 해당하는 사원 번호 조회
	  --> MANAGER_ID 컬럼에 작성되어 있는 사번을 가진 사원 == 사수
...
SELECT EMP_ID, EMP_NAME, NVL(DEPT_TITLE,'없음') AS DEPT_TITLE, JOB_NAME,
		CASE
			WHEN EMP_ID IN (SELECT DISTINCT(MANAGER_ID) /*1번 SQL*/ 
											FROM EMPLOYEE
											WHERE MANAGER_ID IS NOT NULL)
			THEN '사수'
			ELSE '사원'
...
-- 상관 서브쿼리는 메인쿼리 1행 해석 -> 서브쿼리 수행 -> 다시 메인쿼리 1행 해석 -> 서브쿼리 수행 무한반복
...
WHERE EXISTS ( -- 서브쿼리 조회 결과가 있으면 TRUE == 해당 행을 결과에 포함
	SELECT '있음'  -- 서브쿼리에서 조회되는 컬럼 값은 중요하지 않음!
...
--	ROWNUM BETWEEN 1 AND 7; -- 1행 이상 7행 이하
	"순위" BETWEEN 3 AND 7; -- 3행 이상 7행 이하
