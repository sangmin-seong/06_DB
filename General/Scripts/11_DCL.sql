...
-- ORA-01045: 사용자 TEST_USER는 CREATE SESSION 권한을 가지고있지 않음; 
-- 로그온이 거절되었습니다
-- DB 접속 권한 없음
 ...
 * 
 * RESOURSE(자원) : 8개 객체 제어 권한을 묶어둔 ROLE
 * 
 * CONNECT(접속)  : DB 접속 권한
 * */
...
SELECT * FROM KH_SSM.EMPLOYEE;

-- 다른 테이블 조회도 되는지 확인
SELECT * FROM KH_SSM.DEPARTMENT;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
   --> 조회 권한이 없어서 테이블 존재여부 조차 모르기 때문에 발생하는 에러
...
SELECT * FROM KH_SSM.EMPLOYEE; -- 조회 불가
