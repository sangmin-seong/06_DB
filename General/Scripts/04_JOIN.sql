...
--> DEPT_CODE와 DEPT_id 값이 같은 행을 한 줄(한 행)으로 합쳐라!!!
...
-- 사번,이름, 부서명을
-- 사번 오름 차순으로 조회
SELECT * FROM EMPLOYEE;					-- 1)서로 다른 테이블을 합치기 위해 		
SELECT * FROM DEPARTMENT; 			--   같은 값을 지닌 컬럼명 찾기

SELECT EMP_ID, EMP_NAME, DEPT_TITLE			-- 5) 표시하고자 하는 값을 가진 컬럼명 
FROM EMPLOYEE														-- 2) 기준이 되는 테이블
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID) -- 3) 합쳐질 테이블, 같은 값을 가진 컬럼명 일치함을 표시 
WHERE DEPT_TITLE = '해외영업2부'						-- 4) 원하는 조건 대입
ORDER BY EMP_ID ASC;										-- 6) 정렬방법 기입
...
SELECT 
	EMP_ID,
	EMP_NAME,
	J.JOB_CODE, -- 어떤 테이블의 컬럼을 조회할지 별칭을 작성
	JOB_NAME
...
WHERE DEPT_CODE = DEPT_ID(+);
-- DEPT_CODE의 값이 
-- DEPT_ID에 일치하지 않아도 결과에 포함(추가)해라
...
-- 사원의 급여가
-- SAL_LEVEL에 작성된 최소 ~ 최대
-- 범위의 급여가 맞을 때만 결과에 포함하겠다는 JOIN

--> 사원이 자신의 급여 레벨에 맞는 돈을 발 받고 있는지 확인
SELECT
	E.EMP_NAME,
	E.SAL_LEVEL,
	S.MIN_SAL,
...
-- EMPLOYEE 테이블에서
-- 모든사원의 이름과 사수번호(MANAGER_ID), 사수 이름(MANAGER_NAME) 조회
...
FROM EMPLOYEE
NATURAL JOIN JOB;
--> 두 테이블에서 같은 컬럼명, 자료형을 지닌 JOB_COD 컬럼 기준으로
--  JOIN이 진행된다
...
-- EMPLOYEE, DEPARTMENT, LOCATION JOIN 하기
-- ANSI 표준
...
SELECT
	...
	YEE.EMP_NAME "동료이름"
FROM EMPLOYEE EMP
LEFT JOIN EMPLOYEE YEE
					ON (EMP.DEPT_CODE = YEE.DEPT_CODE)
WHERE EMP.EMP_NAME != YEE.EMP_NAME
