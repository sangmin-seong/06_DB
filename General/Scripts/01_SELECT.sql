 ...
 * - 작성되는 문자열에 '_' 기호가
 *   LIKE '_' 와일드 카드와 
 * 	 똑같이 인식 되어 구문이 안되는 문제 발생
 * 
 * 	[해결방법]
 *   - LIKE의 ESCAPE OPTION 사용
 * 		 
 * ESCAPE OPTION : 와일트 카드가 안니 단순 문자열로 인식
 * -> 지정된 특수 문자 뒤 "한 글자"에만 적용된다!!
 * 
 * EX) WHERE 컬럼명 LIEK '___#_' ESCAPE '#';
 * 	-> '#' 뒤에 '_'는 일반 문자열로 인식된다
 * */
...
-- 모든 사원 사번, 이름을
-- 부서코드 오름차순(NULLS LAST 기본값)으로 조회
SELECT EMP_ID, EMP_NAME , DEPT_CODE
FROM EMPLOYEE
ORDER BY DEPT_CODE ASC 
				 NULLS FIRST; -- NULL이 제일 위로 올라온다
...
ORDER BY DEPT_CODE DESC 
				 NULLS LAST; -- NULL이 제일 밑으로 내려간다				 


/* 정렬 기준 "중첩" 작성 */
-- 먼저 작성된 큰 그룹 정렬 후
-- 큰 그룹의 정렬이 깨지지 않는 선에서
-- 이후 작성된 작은 그룹을 정렬 (이중 정렬) 
