...
/* 자료형
    ...
    ** CHAR/VARCHAR2 주의사항 **
    ...
    DATE : 날짜 타입 (년월일요일시분초)
    TIMESTAMP : 날짜 타입 (년월일요일시분초 MS 단위 + UTC기준 시차)
    
    BLOB : 대용량 이진 데이터 (바이트 형태로 최대 4GB)
    CLOB : 대용량 문자 데이터 (문자 형태로 최대 4GB)
...
/*
    ...
    2. UNIQUE : 지정된 컬럼에 중복된 값이 존재할 수 없다
    						-> 단, NULL은 가능
    						   왜? 값이 NULL이 아니라 값이 없어서 비교가 안됨 == 중복 판별 불가
    
    3. PRIMARY KEY : 각 행을 구분하기 위한 식별 역할의 컬럼을 지정
    					-> 컬럼에 저장된 값만 알면 모든 행을 구별할 수 있음
    					-> 중복X(UNIQUE) + NULL 허용 안됨(NOT NULL)이 합쳐진 특징을 가짐
    					-> 각 테이블에 1개만 존재가능
    							( 한 컬럼 X, PK 제약 조건이 1개) 
    
    4. FOREIGN KEY : 자식 테이블의 한 컬럼에 작성될 수 있는 값은 
    								 부모 테이블의 한 컬럼(PRIMARY KEY, UNIQUE)에 작성된 값만 쓸 수 있다
										 -> 자식 테이블이 부모 테이블을 참조
										  - 부모 - 자식 관계 형성
										    -> 두 테이블에 같은 종류의 데이터를 지닌 컬럼이 생성)  == JAVA INTERFACE
										       ==> JOIN의 연결 기준이 될 수 있다.
										 데이터 무결성을 위해 중복되지 않은 데이터만 가지고 있는데, FK 제약 조건을 통해 데이터를 설계 관리한다
										 
										 [삭제 옵션]
										 1) ON DELETE SET NULL(부모 삭제 시, 자식은 빈칸) 
										 		: 부모 행을 삭제하면 참조하던 값을 가지고 있는 자식 행에 컬럼 값을 NULL로 변경(SET)
										 		 
										 2) ON DELETE CASCADE(부모 삭제 시, 자식도 삭제)
										 		: 부모 행을 삭제하면 참조하던 값을 가지고 있는 자식 행을 삭제
										 		
    5. CHECK : 컬럼에 지정된 값만 저장할 수 있게 하는 제약조건
    					 - FK 제약조건의 하위 호환  == JAVA IMPORT
...
REFERENCES DEPARTMENT; -- 컬럼명 미작성시 자동으로 PK컬럼 참조
--== DEPATMENT(DEPT_ID)
